...
class ScenarioGeneratorAPI:
    ...
    def generate(self, input_file_name, num_vehicles, duration):
        logger.info("=== GERAÇÃO API: MAPA DIFERENCIADO (api_mapa_validacao.html) ===")
        ...
        # GERA MAPA ESPECÍFICO API
        self._gen_web_map_supabase(lat, lon, bbox, roads_data, validation_dir / "api_mapa_validacao.html")
        ...
        logger.info("=== CONCLUÍDO (API) ===")

    def _run_db_sync(self):
        sync = PROJECT_ROOT / "tcc_sumo" / "src" / "sync_db.py"
        if sync.exists():
