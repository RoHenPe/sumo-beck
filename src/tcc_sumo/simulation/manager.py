...
SB_URL = "https://rumhqljidmwkctjojqdw.supabase.co"
SB_KEY = "sb_publishable_KYQSvBlqUw9hrC0zeB-3Tg_SIdo84So"
...
class SimulationManager:
    ...
    def _load_device_states(self):
        ...
        # Mapeamento inicial (Assume active)
        ...
        # Lógica de Condição: Só conecta se for API
        if HAS_SUPABASE and self.scenario_name == 'api':
            try:
                ...
                for r in res.data:
                    ...
                    if tp == 'SEMAFARO' and mac in mac_to_sumo:
                        sid = mac_to_sumo[mac]
                        if sid in self.device_map: 
                            self.device_map[sid]['status'] = st
                            
                    # Atualiza Câmera (aninhada no semáforo)
                    if tp == 'CAMERA':
                        for sid, dev in self.device_map.items():
                            if dev.get('camera', {}).get('id') == mac:
                                dev['camera']['status'] = st
                                
                logger.info("Estados sincronizados via Supabase (Modo API).")
            except Exception as e:
                logger.warning(f"Erro Supabase (Usando local): {e}")
        else:
            # Modo OSM ou sem lib: Tudo continua 'active' como veio do JSON
            logger.info("Modo OSM (Offline): Usando estados padrão (Active).")
    ...
    def _collect_stats(self, step):
        for tid in self.global_stats.keys() if self.global_stats else traci.trafficlight.getIDList():
            ...
            # Se a câmera estiver offline, não coleta dados
            if dev and dev.get('camera', {}).get('status') != 'active': 
                continue
